...
static int digits(char *tmp, unsigned int v, unsigned int base, int upper)
{
    ...
    switch (base) {
    case 10:
        while (v) { tmp[n++] = (char)('0' + umod10(v)); v = udiv10(v); }
        break;
    case 16:
        while (v) { tmp[n++] = d[v & 15u]; v >>= 4; }
        break;
    case 8:
        while (v) { tmp[n++] = (char)('0' + (v & 7u)); v >>= 3; }
        break;
    case 2:
...
static int vfmt(sink_t *s, const char *fmt, va_list ap)
{
    ...
    while (*fmt) {
        ...
        case 'd': case 'i': {
            ...
            if (sv < 0) { isneg = 1; uv = (unsigned int)(-(sv + 1)) + 1u; }
            else        { uv = (unsigned int)sv; }
            base = 10;
            break;
        }
        case 'u': uv = va_arg(ap, unsigned int); base = 10; break;
        case 'x': uv = va_arg(ap, unsigned int); base = 16;
                  if (flags & FL_HASH) pre = "0x"; break;
        case 'X': uv = va_arg(ap, unsigned int); base = 16; upper = 1;
                  if (flags & FL_HASH) pre = "0X"; break;
        case 'o': uv = va_arg(ap, unsigned int); base = 8;
                  if (flags & FL_HASH) pre = "0"; break;
        case 'b': uv = va_arg(ap, unsigned int); base = 2;
                  if (flags & FL_HASH) pre = "0b"; break;
        case 'p': uv = va_arg(ap, unsigned int); base = 16; pre = "0x";
                  if (prec < 0) prec = 8; break;
        case 'c': {
            char c = (char)va_arg(ap, int);
            if (!(flags & FL_LEFT)) pad(s, ' ', width - 1);
            emit(s, c);
            if (flags & FL_LEFT) pad(s, ' ', width - 1);
            fmt++;
            continue;
        ...
        case 's': {
            ...
            if (!(flags & FL_LEFT)) pad(s, ' ', width - slen);
            for (n = 0; n < slen; n++) emit(s, str[n]);
            if (flags & FL_LEFT) pad(s, ' ', width - slen);
            fmt++;
            continue;
        }
...

        {
            ...
            if ((flags & FL_ZERO) && !(flags & FL_LEFT) && prec < 0) {
                ...
                if (room > 0) zeros += room;
            }

            padlen = width - n - zeros - signlen - prelen;

            if (!(flags & FL_LEFT)) pad(s, ' ', padlen);
            if (sign) emit(s, sign);
            { int k; for (k = 0; k < prelen; k++) emit(s, pre[k]); }
            pad(s, '0', zeros);
            while (n-- > 0) emit(s, tmp[n]);
            if (flags & FL_LEFT) pad(s, ' ', padlen);
        }
    }
    return (int)s->len;
...
{
    sink_t s; va_list ap; int r;
    s.buf = 0; s.cap = 0; s.len = 0;
    va_start(ap, fmt);
    r = vfmt(&s, fmt, ap);
    va_end(ap);
    return r;
...
{
    sink_t s; va_list ap; int r;
    s.buf = buf; s.cap = size; s.len = 0;
    va_start(ap, fmt);
    r = vfmt(&s, fmt, ap);
    va_end(ap);
    if (size) buf[(s.len < size - 1) ? s.len : size - 1] = '\0';
    return r;
}
